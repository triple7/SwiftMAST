...
extension SwiftMAST {
     ...
     */
    public func queryMast(
        service: Service, params: MASTJson, returnType: APIReturnType,
    ...
    /** Downloads a product bundle from MAST
     ...
     closure: (Bool, [URL])
     */
    public func requestProductBundle(
        ...
        var request = URLRequest(
            url: MASTRequest(searchType: .image).getBundleDownloadUrl(service: service))
    ...
    public func getDataproducts(
        targetName: String, service: Service, products: [CoamResult], productType: ProductType,
        token: String?, completion: @escaping ([FitsData]) -> Void
    ) {
        let serialQueue = DispatchQueue(label: "MASTDataproductsQueue")

        var remainingProducts = products.filter {
            ...
            let operation = MASTDownloadOperation(
                ...
                completionHandler: { (data, response, error) in
                    ...
                    if self.requestIsValid(error: error, response: response) {
                        ...
                        if productType == .Fits {
                            self.saveAsset(
                                targetName: targetName, product: product, urlString: productUrl,
                                data: data!,
                                completion: { fits in
                                    if let fits = fits {
    ...
    public func getDirectDataproducts(
        targetName: String, service: Service, products: [CoamResult], productType: ProductType,
        token: String?, completion: @escaping ([URL]) -> Void
    ) {
     ...
     */
    public func downloadPS1Cutouts(
    ...
    /** Forms a request object from the given PS1 request URL and given parameters
         ...
         closure: whether request was successful
         */
    public func queryPS1(ps1Request: PS1Request, _ closure: @escaping (Bool) -> Void) {
        ...
        let task = session.dataTask(with: request) { [weak self] data, response, error in
            if self!.requestIsValid(error: error, response: response) {
                let table = self!.parsePS1table(
                    ...
                    baseUrl: ps1Request.getFitsCutUrlBase())
    ...
    public func getNedResolver(target: String, completion: @escaping (NedResult?) -> Void) {
