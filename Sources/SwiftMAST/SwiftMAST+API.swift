...
func getTicCrossmatch(ra: Float, dec: Float, radius: Float, result: @escaping ([[Float]]) -> Void) {
     ...
     */
    public func getTICBYRaDec(ra1: Double, ra2: Double, dec1: Double, dec2: Double, completion: @escaping (MASTTAPResponse)-> Void) {
        let selectQuery = "SELECT TOP 100000 id,hip FROM dbo.catalogrecord WHERE ra > \(ra1) AND ra < \(ra2) AND dec > \(dec1) AND dec < \(dec2)"
